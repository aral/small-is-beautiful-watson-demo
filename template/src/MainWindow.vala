namespace Myloveydemo {
    public class MainWindow : Hdy.ApplicationWindow {
        public weak Myloveydemo.Application app { get; construct; }

        // Widgets
        public Myloveydemo.Widgets.HeaderBar toolbar;
        ...
        public MainWindow (Myloveydemo.Application application) {
            Object (
                ...
                icon_name: "com.github.aral.small_is_beautiful_watson_demo"
            );
        }
        ...
        private void create_layout () {
            ...
            toolbar = new Myloveydemo.Widgets.HeaderBar ();
        ...
        private void restore_window_state () {
            ...
            Myloveydemo.saved_state.get ("window-size", "(ii)", out rect.width, out rect.height);

            default_width = rect.width;
            default_height = rect.height;

            var window_state = Myloveydemo.saved_state.get_enum ("window-state");
            switch (window_state) {
                ...
                case WindowState.FULLSCREEN:
                    ...
                    break;
                default:
                    Myloveydemo.saved_state.get ("window-position", "(ii)", out rect.x, out rect.y);
        ...
        private void preserve_window_state () {
            ...
            if (Gdk.WindowState.MAXIMIZED in state) {
                Myloveydemo.saved_state.set_enum ("window-state", WindowState.MAXIMIZED);
            } else if (Gdk.WindowState.FULLSCREEN in state) {
                Myloveydemo.saved_state.set_enum ("window-state", WindowState.FULLSCREEN);
            } else {
                Myloveydemo.saved_state.set_enum ("window-state", WindowState.NORMAL);
                // Save window size
                int width, height;
                get_size (out width, out height);
                Myloveydemo.saved_state.set ("window-size", "(ii)", width, height);
            }

            int x, y;
            get_position (out x, out y);
            Myloveydemo.saved_state.set ("window-position", "(ii)", x, y);
        }
